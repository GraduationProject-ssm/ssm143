...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1T48XecE9G?p=138)
...
# 4 系统设计
校园一卡通系统的设计方案比如功能框架的设计，比如数据库的设计的好坏也就决定了该系统在开发层面是否高效，以及在系统维护层面是否容易维护和升级，因为在系统实现阶段是需要考虑用户的所有需求，要是在设计阶段没有经过全方位考虑，那么系统实现的部分也就无从下手，所以系统设计部分也是至关重要的一个环节，只有根据用户需求进行细致全面的考虑，才有希望开发出功能健全稳定的程序软件。
## 4.1 系统概要设计
...
|3|exam\_name|String|考试名称|是|
|4|kemu\_types|Integer|科目|是|
|5|fraction|BigDecimal|分数|是|
表4.2字典表表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|6|dic\_code|String|字段|是|
|7|dic\_name|String|字段名|是|
|8|code\_index|Integer|编码|是|
|9|index\_name|String|编码名字|是|
|10|super\_id|Integer|父字段id|是|
表4.3缴费记录表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|11|xfname|String|消费原因|是|
|12|yonghu\_id|Integer|消费人|是|
|13|xfmoney|BigDecimal|消费金额|是|
|14|xiaofei\_time|Date|消费时间|是|
表4.4缴费类型表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|15|name|String|设备名称|是|
|16|money|BigDecimal|缴费金额|是|
表4.5设备借还表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|17|shebei\_id|Integer|设备名称|是|
|18|yonghu\_id|Integer|租借人|是|
|19|zhujie\_time|Date|租借时间|是|
|20|zhujie\_types|Integer|租借状态|是|
|21|guihuan\_time|Date|设备归还时间|是|
表4.6考勤表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|22|qdname|String|签到课堂|是|
|23|yonghu\_id|Integer|签到人|是|
|24|qiandao\_time|Date|签到时间|是|
表4.7设备表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|25|sbname|String|设备名称|是|
|26|sbimg\_photo|String|设备图片|是|
|27|shebei\_types|Integer|设备状态|是|
|28|tongzhi\_content|String|设备信息|是|
表4.8用户表表
...
|35|id\_number|String|身份证|是|
|36|balance|double(10,2)|余额|是|
|37|role|String|身份|是|
表4.9用户表表
...
图5.1 用户信息管理页面
### 5.2 成绩信息管理
如图5.2显示的就是成绩信息管理页面，此页面提供给管理员的功能有：查看已发布的成绩信息数据，修改成绩信息，成绩信息作废，即可删除。
...
图5.2 成绩信息管理页面
### 5.3学科类型管理
如图5.3显示的就是学科类型管理页面，此页面提供给管理员的功能有：根据学科类型进行条件查询，还可以对学科类型进行新增、修改、查询操作等等。
...
图5.3 学科类型管理页面
### 5.1设备借还管理
如图5.4显示的就是设备借还管理页面，此页面提供给管理员的功能有：根据设备借还进行删除、查询操作等等。
